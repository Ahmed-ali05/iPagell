...
| 56 | Le mutazioni classi non hanno una outbox offline. | DEDOTTO | `mutate()` effettua subito `classRequest()` e su failure refresh/throw; nessuna queue/storage mutazioni. | `hooks/use-class-agenda.ts` | È dedotto dall'implementazione ispezionata. |
