...
export class ClassRepository {
  ...
  async update(classId: string, input: { name: string; description: string }) {
    const result = await this.db
      .prepare(
        "UPDATE classes SET name=?,description=?,updated_at=? WHERE id=?",
      )
      .bind(input.name, input.description, Date.now(), classId)
      .run();
  ...
  async revokeInvite(classId: string, inviteId: string) {
    const result = await this.db
      .prepare(
        "UPDATE class_invites SET revoked_at=? WHERE id=? AND class_id=? AND revoked_at IS NULL",
      )
      .bind(Date.now(), inviteId, classId)
      .run();
    if (result.meta.changes !== 1)
  ...
  async join(
    ...
    const results = await this.db.batch([
      this.db
        ...
        .bind(user.id, displayName, now, secretHash, now, user.id),
      this.db
        .prepare(
          `UPDATE class_invites SET uses=uses+1
           WHERE secret_hash=? AND EXISTS (
             SELECT 1 FROM class_members m
             WHERE m.class_id=class_invites.class_id AND m.user_id=? AND m.joined_at=?
           )`,
        )
        .bind(secretHash, user.id, now),
    ]);
    if (results[0].meta.changes !== 1 || results[1].meta.changes !== 1)
      throw new HttpError(
        409,
    ...
    const row = await this.db
      .prepare(
        `SELECT class_id FROM class_members
         WHERE user_id=? AND joined_at=? LIMIT 1`,
      )
      .bind(user.id, now)
  ...
  async updateRole(classId: string, userId: string, role: "moderator" | "member") {
    const result = await this.db
      .prepare(
        "UPDATE class_members SET role=? WHERE class_id=? AND user_id=? AND role!='owner'",
      )
      .bind(role, classId, userId)
      .run();
    if (result.meta.changes !== 1)
  ...
  async transfer(classId: string, ownerId: string, nextOwnerId: string) {
    ...
    const results = await this.db.batch([
      this.db
        .prepare(
          "UPDATE class_members SET role='member' WHERE class_id=? AND user_id=? AND role='owner'",
        )
        .bind(classId, ownerId),
      this.db
        .prepare(
          "UPDATE class_members SET role='owner' WHERE class_id=? AND user_id=? AND role!='owner'",
        )
        .bind(classId, nextOwnerId),
      this.db
        .prepare(
          "UPDATE classes SET owner_id=?,updated_at=? WHERE id=? AND owner_id=?",
        )
        .bind(nextOwnerId, Date.now(), classId, ownerId),
    ]);
  ...
  async removeMember(classId: string, userId: string) {
    const result = await this.db
      .prepare(
        "DELETE FROM class_members WHERE class_id=? AND user_id=? AND role!='owner'",
      )
      .bind(classId, userId)
      .run();
    if (result.meta.changes !== 1)
